...
from apps.users.exceptions import NotAuthorizedException
...
class AuthenticationService:
    ...
    def encode_token(self, user_id: int) -> str:
        return jwt.encode(
            {"user_id": user_id, "exp": self._unix_timestamp(
                seconds_in_future=24 * 60 * 60)},
            self.JWT_SECRET_KEY,
...
class BearerAuth(HttpBearer):
    def authenticate(self, request, token) -> str:
        ...
        if not (user := ServiceUser.objects.filter(user_id=user_id).first()):
            raise NotAuthorizedException
        request.user = user
        return token
